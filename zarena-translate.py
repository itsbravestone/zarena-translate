...
from .services import service_url
...
URLS_SUFFIX = [re.search('translate.google.(.*)', url.strip()).group(1) for url in service_url]
URL_SUFFIX_DEFAULT = 'cn'


class zarena_transError(Exception):
    """Exception that uses context to provide a meaningful error message"""
    ...
    def __init__(self, msg=None, **kwargs):
        ...
        if msg:
            ...
            self.msg = None
        super(zarena_transError, self).__init__(self.msg)
...
class zarena_translator:
    '''
    ©️ Bravestone#7521
    You can use 108 language in target and source,details view lang_list.
    ...
    :param url_suffix: The source text(s) to be translated. Batch translation is supported via sequence input.
                       The value should be one of the url_suffix listed in : `service_url`
    ...
    :param lang_tgt: The language to translate the source text into.
                     The value should be one of the language codes listed in : `lang_list`
    :type lang_tgt: :class:`str`; :class:`unicode`

    :param lang_src: The language of the source text.
                    The value should be one of the language codes listed in :const:`googletrans.lang_list`
    ...
    def translate(self, text, lang_tgt='auto', lang_src='auto', pronounce=False):
        try:
            lang = lang_list[lang_src]
        except:
            lang_src = 'auto'
        try:
            lang = lang_list[lang_tgt]
        except:
            lang_src = 'auto'
        text = str(text)
        if len(text) >= 5000:
        ...
        try:
            ...
            raise zarena_transError(tts=self, response=r)
        except requests.exceptions.RequestException as e:
            # Request failed
            raise zarena_transError(tts=self)
        ...
        try:
            ...
            for line in r.iter_lines(chunk_size=1024):
                ...
                if "MkEWBc" in decoded_line:
                    ...
                    return [detect_lang, lang_list[detect_lang.lower()]]
            r.raise_for_status()
        except requests.exceptions.HTTPError as e:
            # Request successful, bad response
            log.debug(str(e))
